...
public class ReferenteClasseUtenteAssembler extends RepresentationModelAssemblerSupport<UtenteEntity, Referente> {
	...
	public ReferenteAdesioneEntity toEntity(ReferenteCreate src, AdesioneEntity adesione) {
		...
		if(exists) {
			throw new BadRequestException(ErrorCode.UT_404_BY_NAME, java.util.Map.of("nomeUtente", entity.getReferente().getNome(), "cognomeUtente", entity.getReferente().getCognome(), "tipoReferente", entity.getTipo().toString()));
		}
