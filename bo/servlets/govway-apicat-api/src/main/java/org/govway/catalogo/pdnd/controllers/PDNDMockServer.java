...
public class PDNDMockServer {
	...
	private <T> T readMockResponse(Class<T> valueType, String suffix) {
    	try {
	    	...
	    	if(files.containsKey(key)) {
	    		...
	    		throw new BadRequestException(ErrorCode.API_005);  // key);
	    	}
	...
	private void checkInput(String id) {
		...
		if(id.startsWith("eeeee400")) {
			throw new BadRequestException(ErrorCode.API_005);  // "ID ["+id+"]");
		}
		
		if(id.startsWith("eeeee404")) {
			throw new NotFoundException(ErrorCode.GEN_002);  // "ID ["+id+"]");
		}
		
		if(id.startsWith("eeeee500")) {
			throw new InternalException(ErrorCode.SYS_001);  // "ID ["+id+"]");
		}
	}
