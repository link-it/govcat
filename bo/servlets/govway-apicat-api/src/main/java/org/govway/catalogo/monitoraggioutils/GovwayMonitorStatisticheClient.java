	...
	public GetReportResponse getReport(GetReportRequest request) {
		try {
			...
			if(result != null) {
				...
				throw new NotFoundException(ErrorCode.GEN_005, Map.of("tipo", "report"));
			}
		} catch(Exception e) {
...
//					valore.setValore(Long.parseLong(record.get(valoreString)));
		...
		return this.apiService.runTransaction(() -> {
			for(String tipo: tipi) {
				try {
					...
					throw new InternalException(ErrorCode.INT_001, Map.of(), e);
				}
			}
			return map.values();
		});
	...
	private String getGruppo(Evento item) {
		...
		if(split.length == 3) {
			...
			throw new InternalException(ErrorCode.VAL_002, Map.of("origine", item.getOrigine()));
		}
	...
	private ItemServizioVerificato getServizio(String origine, String soggetto, String tipo) {
		...
		if(split.length == 3) {
			...
			throw new InternalException(ErrorCode.VAL_002, Map.of("origine", origine));
		}
	...
	public GetInformazioniPuntualiResponse getInformazioniPuntuali(GetInformazioniPuntualiRequest request) {
		...
		return this.apiService.runTransaction(() -> {
			...
			ApiEntity api = this.catalogoCache.getApiEntity(erogatore, request.getName(), request.getVersion())
					.orElseThrow(() -> new NotFoundException(ErrorCode.API_003, Map.of("nomeApi", request.getName(), "versione", String.valueOf(request.getVersion()), "erogatore", erogatore)));
			...
			if(request.getProvider() != null) {
				...
				if(count <= 0) {
					throw new NotFoundException(ErrorCode.ADE_001, Map.of("nomeSoggetto", request.getProvider(), "nomeServizio", servizio.getNome(), "versioneServizio", servizio.getVersione()));
				}
