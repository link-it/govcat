...
describe('AgidJwtSignatureTrackingEvidenceDialogComponent', () => {
  ...
  it('should initialize from tokenPolicy on ngOnInit', () => {
    ...
    expect(component._audience_audit).toBe('https://aud-audit.it');
    expect(component._editAudienceIntegrity).toBe(false);
    expect(component._editAudienceAudit).toBe(false);
    expect(component._expiration).toBe(20);
    expect(component._tokenUrl).toBe('https://token.it/sig-audit');
    expect(component._loa).toBe('SpidL2');
    ...
    expect(component.formGroup.get('audience_audit')?.value).toBe('https://aud-audit.it');
    expect(component.formGroup.get('loa')?.value).toBe('SpidL2');
  });

  it('should set _editAudienceIntegrity and _editAudienceAudit to true when empty', () => {
    component.tokenPolicy = {
      codice_policy: 'p',
      type: 'JWT',
      alg_default: 'RS256',
  ...
  it('should return form controls via f getter', () => {
    ...
    expect(controls['userLocation']).toBeDefined();
    expect(controls['loa']).toBeDefined();
    expect(controls['digest']).toBeDefined();
    expect(controls['signedToken']).toBeDefined();
    expect(controls['signatureToken']).toBeDefined();
  });
  ...
  it('should set keyFile control and patch form values', () => {
    ...
    expect(component.formGroup.get('audience_audit')?.value).toBe('aud-audit1');
  });
  ...
  it('should close modal with data', () => {
    component.ngOnInit();
    const nextSpy = vi.spyOn(component.onClose, 'next');
    component.closeModal({ digest: 'd', signature: 's', trackingEvidence: 'te', voucher: 'v' });
    expect(nextSpy).toHaveBeenCalledWith({
      close: true,
      result: { digest: 'd', signature: 's', trackingEvidence: 'te', voucher: 'v' }
    });
    expect(mockModalRef.hide).toHaveBeenCalled();
  });
  ...
  it('should call closeModal with digest, signature, trackingEvidence, and voucher from form', () => {
    component.ngOnInit();
    component.formGroup.get('result')?.setValue('voucher-value');
    component.formGroup.get('digest')?.setValue('SHA-256=abc');
    component.formGroup.get('signedToken')?.setValue('tracking-jwt');
    component.formGroup.get('signatureToken')?.setValue('signature-jwt');
    const closeSpy = vi.spyOn(component, 'closeModal');
    component.useResultModal();
    expect(closeSpy).toHaveBeenCalledWith({
      digest: 'SHA-256=abc',
      signature: 'signature-jwt',
      trackingEvidence: 'tracking-jwt',
  ...
  it('should set error when keyFormat is unsupported', async () => {
    ...
    await component.generateJWT({
      kid: 'k1', alg: 'RS256', typ: 'JWT', clientId: 'c1',
      audience: 'aud', audience_integrity: 'aud-i', audience_audit: 'aud-a',
      purposeId: 'p1', key: 'k', userID: 'u1', userLocation: 'loc1', loa: 'SpidL2',
      userPayload: 'payload'
    });
  ...
  it('should handle PEM key error gracefully', async () => {
    ...
    const result = await component.generateJWT({
      kid: 'k1', alg: 'RS256', typ: 'JWT', clientId: 'c1',
      audience: 'aud', audience_integrity: 'aud-i', audience_audit: 'aud-a',
      purposeId: 'p1', key: 'bad-key', userID: 'u1', userLocation: 'loc1', loa: 'SpidL2',
      userPayload: 'payload'
    });
  ...
  it('should handle DER key with both PRIVATE KEY and RSA PRIVATE KEY failures', async () => {
    ...
    await component.generateJWT({
      kid: 'k1', alg: 'RS256', typ: 'JWT', clientId: 'c1',
      audience: 'aud', audience_integrity: 'aud-i', audience_audit: 'aud-a',
      purposeId: 'p1', key: 'derdata', userID: 'u1', userLocation: 'loc1', loa: 'SpidL2',
      userPayload: 'payload'
    });
    expect(component._error).toBe(true);
  ...
  it('should set error when _readKeyFile rejects with Error', async () => {
