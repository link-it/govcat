...
describe('AgidJwtTrackingEvidenceDialogComponent', () => {
  ...
  it('should initialize from tokenPolicy on ngOnInit', () => {
    component.tokenPolicy = {
      ...
      expires_in: 15,
      token_url: 'https://token.it/audit',
      loa: 'SpidL2',
    };
    component.ngOnInit();
    expect(component._codicePolicy).toBe('audit_policy');
    ...
    expect(component._tokenUrl).toBe('https://token.it/audit');
  ...
  it('should set _editAudienceAudit to true when audience_audit is empty', () => {
    component.tokenPolicy = {
      codice_policy: 'p',
      type: 'JWT',
      alg_default: 'RS256',
  ...
  it('should close modal with data', () => {
    component.ngOnInit();
    const nextSpy = vi.spyOn(component.onClose, 'next');
    component.closeModal({ trackingEvidence: 'te', voucher: 'v' });
    expect(nextSpy).toHaveBeenCalledWith({ close: true, result: { trackingEvidence: 'te', voucher: 'v' } });
    expect(mockModalRef.hide).toHaveBeenCalled();
  });

  // --- useResultModal ---

  it('should call closeModal with trackingEvidence and voucher from form', () => {
    component.ngOnInit();
  ...
  it('should set error when keyFormat is unsupported', async () => {
    ...
    await component.generateJWT({
      ...
      audience: 'aud', audience_audit: 'aud-a', purposeId: 'p1',
      key: 'k', userID: 'u1', userLocation: 'loc1', loa: 'SpidL2'
    });
  ...
  it('should handle PEM key error gracefully', async () => {
    ...
    const result = await component.generateJWT({
      kid: 'k1', alg: 'RS256', typ: 'JWT', clientId: 'c1',
      audience: 'aud', audience_audit: 'aud-a', purposeId: 'p1',
      key: 'bad-key', userID: 'u1', userLocation: 'loc1', loa: 'SpidL2'
    });
  ...
  it('should handle DER key with both PRIVATE KEY and RSA PRIVATE KEY failures', async () => {
    ...
    await component.generateJWT({
      kid: 'k1', alg: 'RS256', typ: 'JWT', clientId: 'c1',
      audience: 'aud', audience_audit: 'aud-a', purposeId: 'p1',
      key: 'derdata', userID: 'u1', userLocation: 'loc1', loa: 'SpidL2'
    });
    expect(component._error).toBe(true);
