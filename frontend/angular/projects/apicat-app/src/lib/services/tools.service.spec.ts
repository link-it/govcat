...
describe('Tools', () => {
    ...
    beforeEach(() => {
        ...
        Tools.translate = (service as any).translate;
    ...
    it('formatValue should handle type tag', () => {
        vi.spyOn(GridFormatters, 'typeTagFormatter').mockReturnValue('formatted tag');
    ...
    it('generateFields should handle type download', () => {
        vi.spyOn(Tools, 'getObjectValue').mockReturnValue('download');
    ...
    it('generateFields should handle value exists', () => {
        vi.spyOn(Tools, 'getObjectValue').mockReturnValue('value');
        vi.spyOn(Tools, 'formatValue').mockReturnValue('formatted value');
    ...
    it('generateFields should handle type number', () => {
        vi.spyOn(Tools, 'getObjectValue').mockReturnValue(0);
        vi.spyOn(Tools, 'formatValue').mockReturnValue('formatted number');
    ...
    it('generateFields should handle empty true', () => {
        vi.spyOn(Tools, 'getObjectValue').mockReturnValue(null);
    ...
    it('FileExceedError should handle max provided', () => {
        vi.spyOn(Tools, 'MaxUpload').mockReturnValue(1000);
        vi.spyOn(Tools, 'Alert');
    ...
    it('FileExceedError should handle max and MaxUpload both zero', () => {
        vi.spyOn(Tools, 'MaxUpload').mockReturnValue(0);
        vi.spyOn(Tools, 'Alert');
    ...
    it('should return number format for English language', () => {
        (service as any).translate.currentLang = 'en';
    ...
    it('should return number format for Italian language', () => {
        (service as any).translate.currentLang = 'it';
    ...
    it('should return default number format for other languages', () => {
        (service as any).translate.currentLang = 'es'; // Spanish
    ...
    it('should format date with timestamp for English language', () => {
        ...
        // timestamp parameter is not used in current implementation; DateFormatByLanguage() defaults to false
        expect(result).toBe('2022/12/31');
    });
    ...
    it('should format number to currency for Italian language', () => {
        ...
        const expectedFormatted = new Intl.NumberFormat('it-IT', {
    ...
    it('should return "n/a" for invalid number format', () => {
        vi.spyOn(Intl, 'NumberFormat').mockImplementation(() => { throw new Error('Invalid number format'); });
    ...
    it('should return empty string for invalid base64 string', () => {
        vi.spyOn(console, 'log'); // to prevent console.log in the test output
    ...
    it('should not do anything if data is null', () => {
        const spy = vi.spyOn(Tools, 'ConvertToCSV');
    ...
    it('should create a download link and click it', () => {
        const data = [{ a: 1, b: 2 }];
        const csvData = 'a,b\n1,2\n';
        vi.spyOn(Tools, 'ConvertToCSV').mockReturnValue(csvData);
    ...
    it('should set target to _blank if browser is Safari', () => {
        ...
        const link = createElementSpy.mock.results.at(-1)!.value;
    ...
    it('should handle empty array', () => {
        ...
        // ConvertToCSV accesses data[0] directly, so empty array will throw
        expect(() => Tools.ConvertToCSV(data)).toThrow();
    });
...
describe('IsBase64', () => {
    ...
    it('should return true for valid Base64 string', () => {
        ...
        // The implementation uses str.fromBase64().toBase64() which may not be available
        // in all browser environments; when unavailable, the catch block returns false
        const hasFromBase64 = typeof ('' as any).fromBase64 === 'function';
        expect(result).toBe(hasFromBase64 ? true : false);
    });
